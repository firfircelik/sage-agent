...
class RLMEnabledLLMAgent(LLMAgent):
    ...
    def _optimize_and_generate(
        ...
        optimization = self.rlm.process_query(
            full_prompt,
            provider=self.llm_provider.__class__.__name__.lower().replace(
                "provider", ""
            ),
        ...
        try:
            ...
            if self.rlm:
                self.rlm.remember_interaction(
                    query=full_prompt, response=f"Error: {str(e)}", success=False
                )
            return {
    ...
    def print_rlm_stats(self):
        ...
        print(
            f"Avg Tokens Saved/Request: {stats['average_tokens_saved_per_request']:.2f}"
        )
        ...
        if stats.get("enterprise_stats"):
            ...
            if "improvement" in ent:
                print(
                    f"   Quality Trend: {ent['improvement'].get('quality_trend', 'N/A')}"
                )
            if "advanced_optimizer" in ent:
                print(
                    f"   Advanced Optimizations: {ent['advanced_optimizer'].get('total_optimizations', 0)}"
                )
        print(f"{'='*60}")
