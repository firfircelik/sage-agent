...
class UsagePattern:
    ...
    def get_peak_hours(self) -> List[int]:
        ...
        peak_hours = [hour for hour, count in self.time_patterns.items() if count > avg_usage]
    ...
    def _load(self):
        ...
        try:
            ...
            self.time_patterns = defaultdict(int, {int(k): v for k, v in data.get("time_patterns", {}).items()})
...
class IntelligenceEngine:
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        for keyword in keywords:
            if keyword in self.usage_patterns.query_patterns:
                similar_patterns.append((keyword, self.usage_patterns.query_patterns[keyword]))
    ...
    def _categorize_query(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ['code', 'function', 'class', 'api', 'implement']):
            return "coding"
            ...
            return "debugging"
        elif any(word in query_lower for word in ['design', 'architecture', 'structure']):
            return "design"
    ...
    def _estimate_complexity(self, query: str) -> int:
        ...
        complexity_words = sum(1 for word in ['complex', 'advanced', 'detailed', 'comprehensive'] if word in query.lower())
    ...
    def record_performance(
        ...
        self.usage_patterns.record_query(
            query, provider, model, category, tokens_used
        )
    ...
    def get_insights(self) -> Dict[str, Any]:
        ...
        if self.performance_history:
            avg_tokens = sum(p['tokens_used'] for p in self.performance_history) / len(self.performance_history)
            avg_time = sum(p['response_time'] for p in self.performance_history) / len(self.performance_history)
